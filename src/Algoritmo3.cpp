...
Algoritmo3::Algoritmo3(const Grafo& pG):
...
Algoritmo3::~Algoritmo3()
{
    //destructor
}

Grafo Algoritmo3::ObterArvore() const
{
    return T;
}

vector<int> Algoritmo3::ObterBranches() const
{
    return BT;
}

vector<int> Algoritmo3::ObterGrauBT() const
{
    vector<int> GrauBT;
    for (int v : BT)
    {
...
//Algoritmo Árvore Geradora - Terceira Heurística
void Algoritmo3::Oliveira()
{
    BT.clear();
                ...
                for(int u : G.Adjacentes(p))
                {
                    if(T.BuscarVertice(u) == false || (T.CConexa(u) != T.CConexa(p) && (InBT[u] == 1 || T.Grau(u) == 1)))
                    {
                        ...
                        else
                        {
                            if(peso == dPMin)
                            {
                                if(T.BuscarVertice(uAux) == true and T.BuscarVertice(u) == false)
                                {
                                    v = p;
                                    uAux = u;
                                }
